...
func take_damage(amount : int, knockback_dir : int = 0) -> void:
	...
	if current_health <= max_health * 0.5 and phase == 1:
		phase = 2
		emit_signal("half_health_reached")
...
func enter_phase_two_buffed(correct_answer: bool) -> void:
	spell_speed_multiplier = 1.3 if correct_answer else 2.5
	damage_multiplier = 1.2 if correct_answer else 2.5
	attack_timer.wait_time = 1.2 if correct_answer else 0.4
	can_act = true
	attack_timer.start()
	teleport_timer.start()
