...
func _collect():
	...
	# Notify the level to play dialogue
	var level = get_tree().get_first_node_in_group("final_level")
	if level and level.has_method("on_orb_collected"):
		await level.on_orb_collected()
